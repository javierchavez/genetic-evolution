...
public class CrossoverStrategy implements GeneticStrategy <Population, ArrayList<Being>>
{
  ...
  public CrossoverStrategy(Environment environment, Statistics statistics)
  {
  ...
  private ArrayList<Being> selection3(ArrayList<Being> population)
  {

    ArrayList<Being> newParents = new ArrayList<>();
    ...
    if (statistics.getBestBeing() == null || statistics.getBestFitness() > statistics.getBestBeing().getFitness())
    {

      statistics.setBestBeing(currentGenBestBeing);
      statistics.setBestFitness(currentGenBestBeing.getFitness());
  ...
  private ArrayList<Being> createNextGeneration(ArrayList<Being> population)
  {
    ArrayList<Being> nextGeneration = new ArrayList<>();
    ArrayList<Being> newParents = this.select(population); //use selection3 method for tournament size 3 instead of 2
    Random rnd = new Random();
    Being parent1;
    Being parent2;
    ...
    Being mutatedBest = statistics.getBestBeing();
  ...
  @Override
  public ArrayList<Being> run(Population part)
  {
    generationNumber++;
    ...
    // Need to do this //
    currentGeneration.addAll(part);
    // currentGeneration.addAll(population.getBeings());


    this.populationSize = part.size();
    ...
    ArrayList<Being> nextGeneration;
    float summedFitness;
    ...
    for (int i = 0; i < Math.min(part.size(), currentGeneration.size()); i++)
    {
      if (part.get(i).getFitness() < nextGeneration.get(i).getFitness())
      {
        System.out.println("Replacing with more fit");
        part.replace(i, nextGeneration.get(i));
      }
    }
  ...
  @Override
  public ArrayList<Being> select(Population population)
  {
      ArrayList<Being> newParents = new ArrayList<>();
      Random rnd = new Random();
